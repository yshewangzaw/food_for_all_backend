...
const profileController = {
  ...
  getMyQualification: async (req, res) => {
    try {
      ok(
        res,
        await profileService.getQualificationStatus(
          req.user.id,
          req.query.period,
        ),
      );
  ...
  setStatus: async (req, res) => {
    try {
      ok(
        res,
        await profileService.setStatus(req.params.id, req.body, req.user.id),
      );
    } catch (error) {
      fail(res, 400, error);
    }
  ...
  grantQualification: async (req, res) => {
    try {
      ok(
        res,
        await profileService.grantQualification(
  ...
  revokeQualification: async (req, res) => {
    try {
      ok(
        res,
        await profileService.revokeQualification(
          { userId: req.params.id, ...req.body },
          req.user.id,
        ),
      );
  ...
  rejectKyc: async (req, res) => {
    try {
      ok(
        res,
        await profileService.rejectKyc(
          req.params.id,
          req.user.id,
